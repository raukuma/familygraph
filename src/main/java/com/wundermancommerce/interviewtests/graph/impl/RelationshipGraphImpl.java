...
public final class RelationshipGraphImpl implements RelationshipGraph {
    ...
    private void immediateFamilyMembers(Person person, Set<Person> familyMembers){
        ...
        Set<Relationship> familyRelationships = relationshipGraph.edgesOf(person)
                .stream()
                .filter(p->p.getRelationshipType() == RelationshipType.FAMILY)
                .collect(Collectors.toSet());

        familyRelationships.forEach(
                relationship -> {
                    Person nextPerson = person.equals(relationship.getFirstPerson())
                            ? relationship.getSecondPerson() : relationship.getFirstPerson();

                    if(!familyMembers.contains(nextPerson)){
                        familyMembers.add(nextPerson);
                        immediateFamilyMembers(nextPerson,familyMembers);
                    }
                }
        );
    ...
    /**
     * Function finds the Person associated with the given name then initialises
     * the extended family sets and calls de recursive immediateFamilyMembers method.
     *
     * @return number of family members for given person
     * */
    @Override
